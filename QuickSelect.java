...
public class QuickSelect {
    ...
    // Returns k' th smallest element in arr[l..r] in worst case linear time. ASSUMPTION: ALL ELEMENTS IN ARR[] ARE DISTINCT
    static int kthSmallest(int arr[], int l, int r, int k)
    {
        ...
        if (k > 0 && k <= r - l + 1)
        {
            ...
            int length= pos - l + 1; //length of the left side till pivot element
            // If position is same as k
            if (length == k )
                return arr[pos];
            if (k < length) // If position is more, recur for left
                return kthSmallest(arr, l, pos - 1, k);
            // Else recur for right sub array
            return kthSmallest(arr, pos + 1, r, k - length); // k - length is for adjustment of k on j the right side of array
        }
